...
WRONG_ITERATION_JSON='{"reach":99,"impact":99,"confidence":0.99,"effort":99,"reasoning":{"reach":"WRONG_ITERATION_MARKER","impact":"x","confidence":"x","effort":"x"}}'
...
run_validated_iter_test() {
  ...
  if [[ -n "${validated_filename}" ]]; then
    echo "${FIXTURE_JSON}" > "${run_dir}/iteration-1/output/${validated_filename}"
  fi
